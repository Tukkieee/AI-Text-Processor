...
export const Summarizer = ({messages, selectedMessageId, setmessageContent, setshowModal, messageContent}) => {
    ...
    if (!('ai' in self && 'summarizer' in self.ai)) {
      setmessageContent(prev => ({
        ...
        [messageId]: {
          ...prev[messageId],
          isSummarizing: false,
          summaryError: "Your browser doesn't support the Summarization API."
        }
      }));
      return;
    }
    ...
    try {
      ...
      if (canSummarize === 'no') {
        setmessageContent(prev => ({
          ...
          [messageId]: {
            ...prev[messageId],
            isSummarizing: false,
            summaryError: "Summarizer not available. Please try again later."
          }
        }));
        return;
      }
      ...
      setmessageContent(prev => ({
        ...
        [messageId]: {
          ...prev[messageId],
          isSummarizing: false,
          summaryError: "Something went wrong. Please try again."
        }
      }));
    } finally {
      setshowModal(false);
    ...
    <div className="bg-gray-900/50 p-6 rounded-xl border border-indigo-500/20 w-96">
      ...
      <div className="mt-6 flex justify-end space-x-4">
        <button 
          ...
          className="px-4 py-2 text-sm text-gray-300 hover:text-white border-2 rounded-lg"
        ...
        <button 
          ...
          disabled={messageContent[selectedMessageId]?.isSummarizing}
          aria-busy={messageContent[selectedMessageId]?.isSummarizing}
        >
          {messageContent[selectedMessageId]?.isSummarizing ? 'Summarizing...' : 'Summarize'}
        </button>
      </div>
    </div>
  </div>
